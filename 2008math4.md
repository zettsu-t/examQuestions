...
拙文で挙げた[プログラミング言語(リンク)](https://github.com/zettsu-t/zettsu-t.github.io/wiki/If-programming-languages-were)についてVBA, HTML, CSS以外を使い、C#, VB.NET, Python, Groovy, Scheme, OCaml, Rustでも書きました。Cygwinターミナルから、makeを引数なしで実行すると、ビルド、実行、実行結果の検査を行います。
