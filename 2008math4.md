...
拙文で挙げた[プログラミング言語(リンク)](https://github.com/zettsu-t/zettsu-t.github.io/wiki/If-programming-languages-were)についてVBA, HTML, CSS以外を使い、C#, VB.NET, Perl, Python, Groovy, PHP, Scheme, OCaml, F#, Rustでも書きました。Cygwinターミナルから、makeを引数なしで実行すると、ビルド、実行、実行結果の検査を行います。
...
func q1() : bool
    ...
    foreach ops_seq(all_ops)
        ...
        do cui@print(result.toStr() ~ " = " ~ @expr_to_str(1,4,ops_seq) ~ "\n")
    end foreach
    ret true
end func
...
問4-1の解の並び替えができていませんが、一通り解を列挙できたようです。
