...
拙文で挙げた[プログラミング言語(リンク)](https://github.com/zettsu-t/zettsu-t.github.io/wiki/If-programming-languages-were)についてVBA, HTML, CSS以外を使い、C#, VB.NET, Perl, Python, Groovy, Scheme, OCaml, Rustでも書きました。Cygwinターミナルから、makeを引数なしで実行すると、ビルド、実行、実行結果の検査を行います。
...
2017年3月時点の[TIOBE index](http://www.tiobe.com/tiobe-index)におけるratingsについて、上記の言語の合計は20位以内の言語で50.2%, 50位以内の言語で53.1%です。半分確保するのはなかなか大変です。
